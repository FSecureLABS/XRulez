...
#		include "AbstractService.h"
		...
		static std::pair<ReturnEnum, HRESULT> IsServiceInstalled(std::tstring const& serviceName);
		...
		/// @note std::pair is return instead of XError, because there's no clear distinction between a success and a failure here (e.g. error on phase ClosingHandles).
		static std::pair<ReturnEnum, HRESULT> UninstallService(std::tstring const& serviceName);
		...
		static CppTools::XError<HRESULT> TryStartAsService(AbstractService& theOnlyService, std::tstring const& serviceName);
		...
		static std::pair<Status, HRESULT> CheckStatus(std::tstring const& serviceName);
